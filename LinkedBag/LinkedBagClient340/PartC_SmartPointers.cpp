...
bool LinkedBag<ItemType>::addEnd340(const ItemType& thing) {
	...
	auto tempNode = headPtr;
	auto placeholderNode = new Node<ItemType>();
	std::shared_ptr<Node<ItemType>> tempNode_shared = std::make_shared<Node<ItemType>>();
	tempNode_shared->setItem(thing);
	placeholderNode->setItem(tempNode_shared->getItem());

	while (tempNode->getNext() != nullptr) {
		tempNode = tempNode->getNext();
	}
	if (tempNode->getNext() == nullptr) {
		tempNode->setNext(placeholderNode);
		itemCount++;
	}
	return true;
